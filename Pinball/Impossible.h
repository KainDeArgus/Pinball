...
int Impossible(RenderWindow& window) {
    ...
    const int SpeedPlatform1 = 30;
    const int SpeedPlatform2 = 30;
    const int D_ball = 18;
    const int BallPositionX = (width - D_ball) / 2, BallPositionY = (height - D_ball) / 2;
    ...
    t1.loadFromFile("Images/win.png");
    ...
    t7.loadFromFile("Images/background2.jpg");

    Sprite sBackground(t7), sGameOver(t4), sBall1(t2), sPlatform_1(t6), sPlatform_2(t3), sPlatform_3(t5), sPlatform_4(t3), sWin(t1);
    ...
    while (window.isOpen()) {
        ...
        if (b1_cord.y >= height / 2) {
            ...
            //Обработка действий ПЯТОЙ платформы
            if (ChoosePlatform == 5) {
            ...
            case 4:
                ...
                break;
            case 6:
                //Обработка действий ШЕСТОЙ платформы
